...
class AlphaZero():
	...
	def run_selfplay(self, num_games_per_batch, test_set, state_cache, text="Self-Play"):
		...
		for c in range(num_chunks+1):
			...
			time.sleep(2) # Sometimes it bugs if we dont wait before getting the results
			
			for g in range(games_to_play):
				stats = actor_pool.get_next_unordered(300, True) # Timeout and Ignore_if_timeout
	...
	def run_tests(self, player_choice, num_games, state_cache, show_results=True, text="Testing"):	
		...
		for c in range(num_chunks+1):
			...
			for g in range(games_to_play):
				winner, stats = actor_pool.get_next_unordered(300, True) # Timeout and Ignore_if_timeout
				stats_list.append(stats)
				if winner != 0:
					wins[winner-1] +=1
				bar.next()
